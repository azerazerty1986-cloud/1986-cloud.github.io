...
// ========== الكود الأصلي + الجداول والصور فقط ==========
...
// ========== 4. نظام الإشعارات Toast (إضافة تحسينية) ==========
const ToastSystem = {
...
// ========== 5. دالة مساعدة لاستخراج بيانات المنتج (من الكود الأصلي) ==========
...
// ========== 6. الحصول على اسم القسم (من الكود الأصلي) ==========
...
// ========== 7. جلب المنتجات من تليجرام (الكود الأصلي + الصور) ==========
        ...
        if (data.ok && data.result) {
            ...
            for (const update of updates) {
                if (update.channel_post) {
                    ...
                    // ** جلب المنتجات من الصور مع الشرح (إضافة جديدة) **
                    if (post.photo && post.caption && post.caption.includes('🟣')) {
                        console.log('📸 وجدنا منتج مع صورة:', post.caption);
                        ...
                        products.push({
                            ...
                            merchantName: productData.merchantName || 'المتجر',
                            rating: 4.5,
                            images: [imageUrl],
                            createdAt: new Date(post.date * 1000).toISOString()
                        });
                        
                        console.log(`✅ منتج مع صورة مضاف: ${productData.name} - ${productData.price} دج`);
                    }
                    
                    // ** جلب المنتجات من النصوص العادية (الكود الأصلي) **
                    else if (post.text && post.text.includes('🟣')) {
                        console.log('📦 وجدنا منتج نصي:', post.text);
                        ...
                        console.log(`✅ منتج نصي مضاف: ${productData.name} - ${productData.price} دج`);
                    }
                }
            }
        }
...
// ========== 8. جلب التجار من تليجرام (الكود الأصلي) ==========
...
// ========== 9. رفع الصور إلى تلجرام (إضافة جديدة) ==========
...
// ========== 10. معالج رفع الصور (إضافة جديدة) ==========
async function handleImageUpload(event) {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        reader.onload = function(e) {
            preview.innerHTML += `
                <img src="${e.target.result}" class="preview-image" style="width:100px;height:100px;object-fit:cover;margin:5px;border:2px solid var(--gold);border-radius:10px;">
            `;
        };
...
// ========== 11. إضافة منتج إلى تليجرام مع جدول منسق (إضافة جديدة - لا تؤثر على العمل) ==========
async function addProductToTelegram(product) {
    try {
        // ** جدول منسق (تحسين بصري فقط) **
        const tableMessage = `
🟣 *⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯*
🟣         *منتج جديد في المتجر*         
🟣 *⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯*
...
├──────────────────┼────────────────────┤
│ 🏷️ *القسم*       │ ${getCategoryName(product.category).padEnd(18)} │
├──────────────────┼────────────────────┤
...
// ========== 12. إرسال طلب انضمام تاجر (الكود الأصلي) ==========
...
// ========== 13. إرسال طلب شراء مع جدول السلة (إضافة جديدة - تحسين بصري) ==========
async function sendOrderToTelegram(order) {
    try {
        // ** جدول السلة (تحسين بصري فقط) **
        let productsTable = `┌────┬────────────────────┬──────┬─────────┐\n`;
...
// ========== 14. الموافقة على تاجر (الكود الأصلي) ==========
...
// ========== 16. نظام إدارة الطلبات (الكود الأصلي) ==========
...
// ========== 18. نظام التحليلات (الكود الأصلي) ==========
...
// ========== 20. دوال المساعدة والإشعارات (من الكود الأصلي) ==========
...
// ========== 21. دوال التاريخ والوقت (من الكود الأصلي) ==========
...
// ========== 22. دوال تقييم النجوم (من الكود الأصلي) ==========
...
// ========== 24. تحميل المنتجات وعرضها (من الكود الأصلي مع إضافة الصور) ==========
...
// ========== 25. إدارة السلة (من الكود الأصلي) ==========
function loadCart() {
...
// ========== 26. إتمام الشراء (من الكود الأصلي مع جدول السلة) ==========
async function checkoutCart() {
    ...
    // ** إرسال طلب أخضر إلى تلجرام مع جدول السلة **
    ...
    ToastSystem.show('✅ تم إرسال الطلب بنجاح', 'success');
    analyticsManager.trackEvent('purchase', { total: total });
}

// ========== 27. دوال التمرير (من الكود الأصلي) ==========
...
// ========== 28. عداد تنازلي (من الكود الأصلي) ==========
...
// ========== 29. أشرطة التقدم (من الكود الأصلي) ==========
...
// ========== 30. عرض تفاصيل المنتج (من الكود الأصلي مع إضافة الصور) ==========
...
// ========== 31. إدارة المستخدمين (من الكود الأصلي) ==========
...
// ========== 32. تسجيل تاجر جديد (من الكود الأصلي) ==========
...
// ========== 33. إضافة المنتجات (من الكود الأصلي مع إضافة الصور) ==========
...
// ========== 34. حفظ المنتج (من الكود الأصلي مع إضافة الصور) ==========
async function saveProduct() {
    ...
    // ** إرسال المنتج مع الصورة إلى تلجرام **
    const sent = await addProductToTelegram(product);
    
    if (sent) {
        ToastSystem.show('✅ تم إضافة المنتج وسيظهر قريباً', 'success');
        ...
        ToastSystem.show('❌ فشل إضافة المنتج', 'error');
    }
}

// ========== 35. لوحة التحكم (من الكود الأصلي) ==========
...
// ========== 36. تأثيرات الكتابة (من الكود الأصلي) ==========
...
// ========== 37. الاستماع لأوامر تلجرام (من الكود الأصلي) ==========
setInterval(async () => {
    try {
